"""Quark Hardware Hybrid Quantum-Classical Pipeline 入口编排器。
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--compiler", default="quarkcircuit",
    ...
    p.add_argument("--open-dd", default="none", choices=["none", "XY4", "CPMG"],
                   help="dynamical decoupling; none 会传 None")
    p.add_argument("--target-qubits", default="",
                   help="物理比特映射,逗号分隔,例如 '0,1'; 留空表示由后端决定")
    p.add_argument("--poll-interval", type=float, default=1.0, help="Quark 任务状态轮询间隔(秒)")
...
def main() -> int:
    ...
    print(f"  config        : n={args.n}, train={args.train}, test={args.test}, gap={args.gap}, "
          f"reps={args.reps}, C={args.c}, gamma={args.rbf_gamma}, seed={args.seed}, "
          f"chip={args.chip}, shots={args.shots}, compiler={args.compiler}, "
          f"correct={args.correct}, open_dd={args.open_dd}, "
          f"target_qubits={args.target_qubits or '[]'}")
    print("  hardware workload default: K_train off-diagonal + K_test only; no decision grid")
    ...
    timings["03"] = run_task(
        "Task 03 / compute quantum kernel matrix on Quark hardware", TaskKind.QUANTUM,
        "task_03_quantum_kernel.py",
        ["--input", str(DATA_NPZ), "--output-dir", str(KERNEL_DIR),
         "--reps", str(args.reps), "--seed", str(args.seed),
         "--chip", args.chip, "--shots", str(args.shots),
         "--compiler", args.compiler, "--open-dd", args.open_dd,
         "--target-qubits", args.target_qubits,
         "--poll-interval", str(args.poll_interval)]
        + (["--correct"] if args.correct else []),
    )
    ...
    timings["08"] = run_task(
        "Task 08 / final visualization (accuracy bars only)", TaskKind.CLASSICAL,
